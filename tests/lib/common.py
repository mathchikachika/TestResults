...
def set_image_file(file_path: str, file_name: str) -> list:
    ...
    try:
        ...
        image_file: list = [
            (
                f"{file_image_split[0]}",
                (
                    f"{file_name}",
                    open(f"{tests.CURRENT_DIR}\\tests\\images\\{file_name}", "rb"),
                    f"image/{file_image_split[1]}",
                ),
            )
        ]
...
def make_put_request(guid: dict, payload_str: str):
    ...
    req = Requester()
    response = req.mw_put(
        endpoint=f"member/update/{guid}", payload=payload_str, headers=headers
    )
    return response
...
def fetching_member_to_update(member_data) -> dict:
    ...
    headers: dict = create_header()
    req = Requester()
    fetched_member_to_update = req.mw_get(
        f"member/fetch/{member_to_update['guid']}", headers=headers
    )
...
def verify_not_created(
    response, expected_status_code: int, expected_type: str, expected_message: str
):
...
def get_admin_token() -> str:
    token: str = generate_token.generate_token(
...
def get_staar_random_payload_data() -> dict:
    ...
    random_response_type: str = random.choice(
        ["Open Response Exact", "Range Open Response", "Multiple Choice", "Checkbox"]
    )
    ...
    random_letter: str = random.choice(
        ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j"]
    )
    random_unit: str = random.choice(
        [
            "pounds",
            "kilograms",
            "meters",
    ...
    random_keyword: str = random.choice(
        [
            "math",
            "science",
            "history",
    ...
    random_payload: dict = {
        ...
        "random_response_type": random_response_type,
        "random_question_type": random_question_type,
        "random_grade_level": random_grade_level,
        "random_letter": random_letter,
        "random_unit": random_unit,
        "random_is_answer": random_is_answer,
        "random_category": random_category,
        "random_keyword": random_keyword,
        "random_student_expectation": random_student_expectation,
...
def get_mathworld_random_payload_data() -> dict:
    ...
    random_topic: str = random.choice(
        [
            "Algebra 1",
            "Algebra 2",
            "Algebra 3",
    ...
    random_subject: str = random.choice(
        [
            "Math",
            "Science",
            "Social Studies",
    ...
    random_payload: dict = {
        ...
        "random_question_content": random_question_content,
        "random_teks_code": random_teks_code,
    }
    return random_payload
...
def create_a_staar_question(
    member_type: str, response_type: str = "Open Response Exact"
) -> dict:
    try:
        req = Requester()
        user_token: str = (
...
def create_a_mathworld_question(
    member_type: str, response_type: str = "Open Response Exact"
) -> dict:
    try:
        req = Requester()
...
def register_staff_member() -> dict:
    ...
    payload: dict = (
        '{"first_name": "'
        + random_data["first_name"]
        + '", "middle_name": "'
        + random_data["middle_name"]
        + '", "last_name": "'
        + random_data["last_name"]
        + '", "role": "'
        + random_data["role"]
        + '", "school": "'
        + random_data["school"]
