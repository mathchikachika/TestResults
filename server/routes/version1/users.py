...
async def get_user_data(request: Request):
    try:
        ...
        model = User
        projectionModel = (
            Student if role == "student" else Teacher if role == "teacher" else User
        )
...
async def update_account_details(
    request: Request, updated_account: UpdatedUserViaSubscriber
):
    try:
...
@router.patch(
    "/update/student/contact_person",
    dependencies=[Depends(JWTBearer(access_level="student"))],
    status_code=status.HTTP_200_OK,
)
async def update_contact_person(
    request: Request, updated_contact_person: ContactPerson
):
    user_id = request.state.user_details["uuid"]
    role = request.state.user_details["role"]
    if role != "student":
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, detail="User must be a student"
        )
    try:
        fetched_account = await User.get(user_id)
        if fetched_account:
...
async def update_class(request: Request, updated_education: EducationList):
    ...
    if role != "teacher":
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, detail="User must be a teacher"
        )
    try:
