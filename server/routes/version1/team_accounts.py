...
async def get_user_data(request: Request, account_id: str = None):
    ...
    try:
        account = (
            await Account.find({"_id": ObjectId(account_id)})
            .project(AccountResponseModel)
            .to_list(None)
        )
        return account
    except Exception as e:
...
async def update_account_details(
    request: Request,
    account_id: str = None,
    updated_account: Any = Body(
        openapi_examples=sample_payloads.updated_account_payload
    ),
):
    ...
    try:
        ...
        if fetched_account:
            account = await fetched_account.update({"$set": updated_account.dict()})
...
async def delete_account(account_id: str):
    try:
        deleted_account = await db["account_collection"].find_one_and_delete(
