...
async def login(credentials: LogIn):
    try:
        account = await SubscriberAccount.find_one(
            SubscriberAccount.email == credentials.email
        )
        if account:
        ...
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, detail="username or password is incorrect"
        )
        ...
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, detail="An error occured: " + str(e)
        )
...
async def register_emails(request: Request, user_accounts: UserAccounts):
    try:
        ...
        accounts = (
            await User.find(In(User.id, results.inserted_ids))
            .project(InitialUserAccountResponseModel)
            .to_list()
        )
...
async def get_all_users(
    ...
    try:
        ...
        accounts = (
            ...
            .project(UserResponseModel)
            .sort(-User.updated_at)
...
async def get_user_data(request: Request, user_id: str = None):
    try:
        ...
        account = (
            await model.find({"_id": ObjectId(user_id)})
...
async def update_account_details(
    request: Request, updated_account: UpdatedUserViaSubscriber, user_id: str = None
):
    try:
...
async def update_user_role(request: Request, updated_role: UpdatedRole, user_id: str):
    try:
        ...
        if fetched_account:
            await fetched_account.update({"$set": updated_role})
...
@router.patch(
    "/update/user_status{user_id}",
    dependencies=[Depends(JWTBearer(access_level="subscriber"))],
    status_code=status.HTTP_200_OK,
)
async def update_user_status(
    request: Request, updated_status: UpdatedStatus, user_id: str
):
    try:
        ...
        if fetched_account:
            await fetched_account.update({"$set": updated_status})
...
async def change_password(request: Request, updated_password: UpdatedPassword):
    try:
        ...
        if fetched_account:
            verified = Hasher().verify_password(
                login_password=updated_password.old_password,
                member_password=fetched_account.password,
            )
            if verified:
                updated_password = updated_password.model_dump()
                del updated_password["old_password"]
                del updated_password["repeat_new_password"]
                updated_password["password"] = Hasher().hash_password(
                    updated_password["new_password"]
                )
                del updated_password["new_password"]

                await fetched_account.update({"$set": updated_password})
...
async def delete_account(user_id: str):
    try:
        deleted_account = await db["user_collection"].find_one_and_delete(
